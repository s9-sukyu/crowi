...
SearchClient.prototype.getAlias = async function() {
  const existsAlias = await this.existsAlias()
  if (existsAlias) {
    const aliases = await this.client.cat.aliases({ name: this.indexNames.current, format: 'json' })
    if (aliases.length > 0) {
      return aliases[0]
    }
  }
...
SearchClient.prototype.putAlias = function(index) {
  return this.client.indices.putAlias({ index, name: this.indexNames.current })
}

SearchClient.prototype.ensureIndex = async function() {
  const indices = await this.getIndices()
  if (indices.length === 0) {
...
SearchClient.prototype.prepareBodyForUpdate = async function(body, page, index = null) {
  ...
  if (alias === null && index === null) {
    throw new Error('Alias is not found.')
  }
...
SearchClient.prototype.prepareBodyForCreate = async function(body, page, index = null) {
  if (!Array.isArray(body)) {
    throw new Error('Body must be an array.')
  }
...
SearchClient.prototype.prepareBodyForDelete = async function(body, page, index = null) {
  if (!Array.isArray(body)) {
    throw new Error('Body must be an array.')
  }
...
SearchClient.prototype.addPages = async function(pages) {
  ...
  for (const page of pages) {
    ...
    await this.prepareBodyForCreate(body, page)
  }
...
SearchClient.prototype.updatePages = async function(pages) {
  ...
  await Promise.all(
    pages.map(async function(page) {
      await self.prepareBodyForUpdate(body, page)
    }),
  )
...
SearchClient.prototype.deletePages = async function(pages) {
  var self = this
  var body = []

  await Promise.all(
    pages.map(async function(page) {
      await self.prepareBodyForDelete(body, page)
    }),
  )
  ...
  return new Promise((resolve, reject) => {
    ...
    cursor
      .eachAsync(async doc => {
        ...
        await self.prepareBodyForCreate(body, page, index)
